...
const Product = () => {
  ...
  const publicKey = process.env.NEXT_PUBLIC_PAYSTACK_PUBLIC_KEY; //  Paystack public key from environment variables
  const amountInNaira = productData ? productData.offerPrice * 100 : 0; // Convert to kobo

  const paystackConfig = {
    reference: new Date().getTime().toString(), // Unique reference for the transaction
    email: "buyersemail@gmail.com",
    amount: amountInNaira, // Amount in kobo
    publicKey,
    metadata: {
      custom_fields: [
        {
